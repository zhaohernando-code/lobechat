...
- Browserless crawl API: `http://127.0.0.1:13000`
...
`com.codex.lobechat.frontend` points at `~/codex/projects/lobechat/scripts/start-local-frontend.sh` and must remain loaded with `RunAtLoad` + `KeepAlive`. The script starts Docker Desktop when the Docker daemon is unavailable, waits for Compose to become usable, starts the stack, re-runs `scripts/lobehubctl.sh up` whenever `http://127.0.0.1:3210/` fails, recreates `lobe` when the auth/OIDC release health check fails, and recreates `searxng` when the loopback JSON search API on `127.0.0.1:18080` fails. Search/crawl health also validates local Browserless on `127.0.0.1:13000`, because LobeHub page-reading fails when Browserless is not configured.
